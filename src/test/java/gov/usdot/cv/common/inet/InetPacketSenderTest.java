...
public class InetPacketSenderTest {
	...
	private static final byte[] PAYLOAD = new byte[] { (byte)0xde, (byte)0xad, (byte)0xbe, (byte)0xef, (byte)0xca, (byte)0xfe, (byte)0xba, (byte)0xbe };

	AssertionError assertionError = null;
	
	private enum TestCase {
		TestForwardPacketInbound,
		TestSendPacketOutbound,
		TestIPv6ForwardOutbound,
		TestIPv4ForwardOutboundForward,
		TestIPv4ForwardOutboundSend,
		TestIPv6SendOutbound,
		TestIPv4SendOutbound,
	};
	...
	public void testForwardPacketInbound(String hostname) throws UnknownHostException, InetPacketException, InterruptedException {
	...
	public void testSendPacketOutbound(String hostname) throws UnknownHostException, InetPacketException, InterruptedException {
		...
		DatagramPacket packet = new DatagramPacket(bundle, bundle.length, InetAddress.getByName(FORWARDER_HOST), FORWARDER_PORT);
		sender.send(packet);
		checkBackgroundThreadAssertion();
	}
	...
	public void testIPv4ForwardOutboundSend() throws InterruptedException, UnknownHostException, InetPacketException {
		// Use case: Transport or Data Sink send IPv4 message out via forwarder but it's being send out directly
		...
		Thread listener = new Thread( new Runnable() {
			...
			public void run() {
				...
				try {
					...
					log.error(String.format("Caught socket timeout exception while recieving message on port %d. Max size is %d", listenPort, DEFAULT_MAX_PACKET_SIZE), ex);
		            assertTrue(false);
